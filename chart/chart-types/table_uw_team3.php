...
<a href="<?php echo BaseURL() .'bar_chart_uw_team3.php?team='.$team.'&tglawal='.$tglawal.'&tglakhir='.$tglakhir;?>" class="next">Chart Ticket Status</a>
<hr />
    ...
    if($status=='approve'){
        echo'
         ...
         echo'
          ...
          <option value="assessed" selected>Assessed</option>';

    }else{
        echo'
          <option value="risk_accepted" selected >Risk_Accepted</option>
          <option value="approve" >Approve</option>
          <option value="assessed" >Assessed</option>';
    }
    ?>
...
if($status=='approve'){
            ...
            select a.sales_number,a.revision_number, a.sales_status as sales_status_now,b.sales_status,c.ticket_number,b.userid,c.ticket_type,b.actiontime from tbl_so_sales_details as a
            ...
            where b.sales_status in('Approve')
            ...
            and b.userid in('dwi.prasetyo','trivina.setyawati','rini.aryani','lisbet')
             ...
             and b.actiontime::date <='$tglakhir'
                        ORDER BY b.userid asc, b.actiontime desc")or die(pg_last_error($nextg_conn));
       ...
       $Assessed = pg_query($nextg_conn, "
            ...
            join tbl_so_sales_tracker as b on a.sales_number=b.sales_number and a.revision_number=b.revision_number
            join tbl_helpdesk_integration as c on a.sales_number=c.sales_number and a.revision_number=c.revision_number
            where b.sales_status in('Assessed')
            and c.ticket_type='Risk Approval'
            and b.userid in('dwi.prasetyo','trivina.setyawati','rini.aryani','lisbet')
             and b.actiontime::date >='$tglawal'
             and b.actiontime::date <='$tglakhir'
       ...
       $Risk_Accepted = pg_query($nextg_conn, "
            ...
            join tbl_so_sales_tracker as b on a.sales_number=b.sales_number and a.revision_number=b.revision_number
            join tbl_helpdesk_integration as c on a.sales_number=c.sales_number and a.revision_number=c.revision_number
            where b.sales_status in('Risk Accepted')
            and c.ticket_type='Risk Approval'
            and b.userid in('dwi.prasetyo','trivina.setyawati','rini.aryani','lisbet')
             and b.actiontime::date >='$tglawal'
             and b.actiontime::date <='$tglakhir'
...
function myStatus() {
 ...
 //alert (tglawal);
  var link='http://dashboard.asuransibintang.com/dashboard/chart/chart-types/table_uw_team3.php?team='+team;
